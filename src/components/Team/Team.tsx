...
const Team = ({ match }: RouteChildrenProps<TeamParams>) => {
  ...
  return (
    <TeamGrid>
      ...
      <img
        className="logo"
        alt={`Logo for the dota2 team ${team.name}`}
        src={team.images.default}
      />
      ...
      <MatchesGrid className="matches">
        ...
        <Paragraph>
          {team.name} is a team from {team.country.name} that participated in{' '}
          {matches?.past.length || 0} matches for the DotA Pro Circuit 2020,
          winning {matches?.won || 0} of them while losing {matches?.lost || 0};
          They achieved the {Placement[position]} position on the League. As of
          now, {team.name} is{' '}
          {getRosterStatus(
            sortedRosters[position].dpc_points,
            position
          ).toLowerCase()}{' '}
          to the International, which has been postponed due to COVID-19.
        </Paragraph>

        {(matches?.future.length || 0) > 0 && (
          <Paragraph>
            The team has {matches?.future.length} scheduled matches to try to
            increase their DotA Pro Circuit points.
          </Paragraph>
        )}
